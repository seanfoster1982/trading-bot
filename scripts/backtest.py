"""Backtester - replays the LIVE strategy against historical candles.

Imports strategy.py and monkeypatches its two data-access functions to read
point-in-time data filtered by a simulated clock. Every downstream strategy
function then operates on historical data automatically, guaranteeing the
backtest runs the SAME logic as the live bot.

Usage:
  python scripts/backtest.py --token TROLL --days 20
  python scripts/backtest.py --all --days 20
"""
...
_KEYS = ["close","macd_line","macd_signal","macd_hist","stoch_rsi_k","stoch_rsi_d",
...
def pit_get_latest_indicators(address, interval):
...
def pit_get_previous_indicators(address, interval, n_back=1):
    conn = sqlite3.connect(DB_PATH)
    rows = conn.execute("""
        SELECT close, macd_line, macd_signal, macd_hist, stoch_rsi_k, stoch_rsi_d,
               bb_upper, bb_middle, bb_lower, ema_50, ema_200, atr_14,
    ...
    if len(rows) <= n_back:
        return None
    return dict(zip(_KEYS, rows[n_back]))


def load_5m_candles(address, since_ts):
    conn = sqlite3.connect(DB_PATH)
    rows = conn.execute("""
...
def simulate_token(symbol, address, days, strat_mod):
    ...
    for idx in range(200, len(candles)):
        ...
        if in_position:
            if l <= stop:
                ...
                trades.append(BTTrade(symbol, entry_price, stop, pnl, "STOP"))
                in_position = False
                last_stop_exit_ts = ts
            elif target and h >= target:
                pnl = (target - entry_price) / entry_price
                trades.append(BTTrade(symbol, entry_price, target, pnl, "TARGET"))
                in_position = False
            continue
        ...
        entry_price = c
        stop = entry_price - (atr * ATR_STOP_MULT) if (atr and atr > 0) else entry_price * 0.95
...
def main():
    ...
    if args.token:
        ...
        targets = conn.execute("""
            SELECT s.symbol, c.address
            FROM candles c
            JOIN (SELECT DISTINCT address, symbol FROM screened_tokens) s
