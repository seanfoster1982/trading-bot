...
content = '''"""Jupiter quote viewer - shows what trades would fill at given current liquidity.

Read-only. No signing. No wallet interaction.

For each pending BUY signal in the signals table:
  1. Query Jupiter quote API: swap N USDC for token X
  2. Show expected output amount + price impact + route
  3. Compare to strategy entry price - flag if slippage too high

Run this whenever you want to see what real fills would look like.
"""
...
async def main_async(test_size_usd):
    ...
    if not signals and test_size_usd:
        msg = "[yellow]No pending BUY signals. Running synthetic quotes at $" + f"{test_size_usd}" + " per token.[/yellow]\\n"
        console.print(msg)
        conn = sqlite3.connect(DB_PATH)
        rows = conn.execute("""
    ...
    if not signals:
        ...
        console.print("  Either wait for the strategy to fire a BUY, or run with:")
    ...
    async with httpx.AsyncClient() as client:
        for i, sig in enumerate(signals, 1):
            status_pref = f"[dim]({i}/{len(signals)}) {sig['symbol']}...[/dim]"
            console.print(status_pref, end="")
...
Path('scripts/jupiter_quote.py').write_text(content, encoding='utf-8')
print('Wrote scripts/jupiter_quote.py')
